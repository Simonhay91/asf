...
**Цель:** 6 районов/день (3+3 в 09:00 и 18:00 МСК) + **1 блог/день** в 09:00 (очередь из seed, в т.ч. «площадка цена»).
...
Блог в cron (по умолчанию включён, 1 статья в 09:00 МСК):
- `CRON_BLOG_COUNT=0` — отключить блог
- `CRON_BLOG_WEEKDAYS=1` — блог только пн–пт (районы всё равно 2× в день)

Очередь блога — следующий pending из MongoDB (не фиксированный slug; «площадка цена» выйдет, когда дойдёт очередь).
```
